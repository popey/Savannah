...
def reset_password(request, request_key):
    try:
        ...
        if reset.expires >= datetime.datetime.now(datetime.timezone.utc):
            if request.method == "POST":
...
class SavannahView:
    def __init__(self, request, community_id):
        ...
        if request.user.is_superuser:
            ...
            self.community = None
        if request.user.is_authenticated:
            self.manager_profile, created = ManagerProfile.objects.update_or_create(user=request.user, community=self.community, defaults={'last_seen': datetime.datetime.now(datetime.timezone.utc), 'send_notifications':(not request.user.is_superuser)})
    ...
    def _add_sources_message(self):
        if self.request.method == "GET" and self.request.user.is_authenticated:
            if self.community.status == Community.SUSPENDED:
                ...
                if self.community.created <= datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=1):
...
class SavannahFilterView(SavannahView):
    def __init__(self, request, community_id):
        ...
        self.rangestart = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=self.timespan)
        self.rangeend = datetime.datetime.now(datetime.timezone.utc)
        self.DATE_FORMAT = '%Y-%m-%d'
    ...
    def is_filtered(self):
        if self.filter['timespan'] and (self.timespan != self.MAX_TIMESPAN or self.rangeend.date != datetime.datetime.now(datetime.timezone.utc).date):
            return True
